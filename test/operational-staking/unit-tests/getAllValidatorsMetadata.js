...
describe('Get all validators metadata', function() {
  it('Should return correct validator addresses', async function() {
    ...
    await addEnabledValidator(0, contract, opManager, VALIDATOR_1, 200);
    await addEnabledValidator(0, contract, opManager, VALIDATOR_2, oneToken.sub(1));
  ...
  it('Should return correct disabled at block number', async function() {
    ...
    expect(md.disabledAtBlocks[0]).to.equal(0);
    await contract.connect(opManager).disableValidator(0, 10);

    md = await contract.getAllValidatorsMetadata();
    expect(md.disabledAtBlocks[0]).to.equal(10);
    ...
    expect(md.disabledAtBlocks[0]).to.equal(10);
    expect(md.disabledAtBlocks[1]).to.equal(0);
    expect(md.disabledAtBlocks[2]).to.equal(0);
    expect(md.disabledAtBlocks[3]).to.equal(0);

    expect(md.disabledAtBlocks.length).to.equal(4)
  });
});
