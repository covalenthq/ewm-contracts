const {
  ...
  describe('Tests startOperatorRole()', function() {
    ...
    it('Lets a validator enable an operator if the validator is preapproved', async function() {
      await proofChain.connect(validators[0]).enableBRPOperator(operators[0].address,);
    });
    ...
    it('Does not let enable an operator that does not exist or performs a different role', async function() {
      ...
      await expect(proofChain.connect(validators[validatorID]).enableBRPOperator(validators[validatorID].address)
    ...
    await expect(proofChain.connect(validators[0]).enableBRPOperator( delegators[0].address),
        ).to.be.revertedWith('Operator does not perform the requested role');

      await proofChain.connect(owner).removeBRPOperator(operators[validatorID].address);
    ...
    it('Should enable validator with correct block number', async function() {
       await expect(
            proofChain.connect(validators[0]).enableBRPOperator( operators[0].address))
